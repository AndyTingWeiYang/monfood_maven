...
@WebServlet("/ResGetRateServlet")
public class ResGetRateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException { response.setCharacterEncoding("UTF-8");
		...
		try {
			...
			List<Map<String, Object>> rateList = service.getRate();

			// add the list into json format
			respObj.add("resList", gson.toJsonTree(rateList));
		} catch (Exception e) {
			e.printStackTrace();
