...
//取出上傳的時間		
		...
		Timestamp startDate = Timestamp.valueOf(sdString);          
		
		JsonElement edElement = json.get("endDate");
		String edString = String.valueOf(edElement);
		Timestamp endDate = Timestamp.valueOf(edString);
...
//取出登入資訊
		...
		List<String> result = service.getDelRecord(startDate, endDate, loginDelVO.getDelID());
